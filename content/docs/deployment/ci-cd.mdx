...
| `publish-cli.yml` | Release published (`create-hyperion@v*`) | Publish CLI to npm |
...
```json title="release-please-config.json (excerpt)"
{
  ...
  "packages": {
    ...
    "packages/cli": {
      ...
      "component": "create-hyperion"
    }
  }
}
```

When a release is created, all packages bump to the same version, including `package.json`, `tauri.conf.json`, and `Cargo.toml`. The CLI (`create-hyperion`) is versioned independently with `separate-pull-requests: true`, so it gets its own Release PR.
...
`publish-cli.yml` triggers automatically when a release tagged `create-hyperion@v*` is published. It builds the CLI with `pnpm --filter create-hyperion build` and publishes to npm with `--provenance` for supply chain security.
