...
Hyperion is a production-grade starter template for building cross-platform apps from a single codebase. One set of pages and components ships to web (SSR + PWA), desktop (Windows, macOS, Linux), and mobile (Android, iOS). End users scaffold a new project with `npm create hyperion@latest`, which clones the template, renames identifiers, and optionally initializes git and installs dependencies.
...
The CLI package (`packages/cli`, published as `create-hyperion` on npm) is independently versioned. CI/CD runs through GitHub Actions with Release Please for Conventional Commits-based versioning and automated changelogs.
...
packages/
  ...
  cli/                Scaffolding CLI: `npm create hyperion@latest` (published to npm)
...
- The CLI package (`packages/cli`) uses Vitest. Run with `pnpm --filter create-hyperion test`.
