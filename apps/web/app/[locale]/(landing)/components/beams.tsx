...
// biome-ignore lint/performance/noNamespaceImport: THREE namespace is required for extension functions
...
function extendMaterial(
  // biome-ignore lint/suspicious/noExplicitAny: required for general class signature
  BaseMaterial: new (...args: any[]) => THREE.Material,
  cfg: MaterialConfig
) {
  ...
  // biome-ignore lint/suspicious/noExplicitAny: library property defines is not explicitly typed
  const baseDefines = (physical as any).defines ?? {};

  // biome-ignore lint/suspicious/noExplicitAny: uniforms properties are dynamic
  const uniforms = THREE.UniformsUtils.clone(baseUniforms) as any;

  // biome-ignore lint/suspicious/noExplicitAny: required to read properties dynamically
...
// biome-ignore lint/suspicious/noReactForwardRef: using React 18 compatible forwardRef signature
const MergedPlanes = forwardRef<THREE.Mesh, MergedPlanesProps>(
  ({ material, width, count, height }, ref) => {
    const mesh = useRef<THREE.Mesh>(null);
    // biome-ignore lint/style/noNonNullAssertion: mesh.current is guaranteed to exist after mount
    ...
    useFrame((_, delta) => {
      if (mesh.current) {
        // biome-ignore lint/suspicious/noExplicitAny: material is dynamically accessed for uniforms
