...
export function AgentInfoPanel({ dev, agentId, index }: AgentInfoPanelProps) {
  ...
  useGSAP(
    () => {
      ...
      const contribEl =
        container.querySelector<HTMLElement>(".gsap-contrib-text");
      ...
      if (bioLabel) {
        tl.from(bioLabel, { y: 12, autoAlpha: 0, duration: 0.18 }, "-=0.06");
      }
      if (bioEl) {
      ...
      if (socials) {
        tl.from(socials, { y: 8, autoAlpha: 0, duration: 0.18 }, "-=0.08");
      }
    },
