---
title: CLI Package
description: Scaffolding CLI. Downloads, renames, and configures Hyperion projects.
icon: Terminal
---
...
Published to npm as [`create-hyperion`](https://www.npmjs.com/package/create-hyperion). Downloads the latest source, renames all identifiers to match your project, configures Android and iOS packaging, inits git, and optionally installs dependencies.
...
```bash
npm create hyperion@latest --name <your-project-name> --github-user <your-github-username> --no-install --no-git
```
...
Downloads the source from `github:BhagirathsinhRana378/hyperion` using giget. The branch defaults to `master` but can be overridden with `--branch`. No git history is carried over.
...
This ensures the scaffolded project starts with a clean documentation structure instead of the Hyperion source docs.
...
|---|---|
| `com.hyperion.app` | User's app identifier |
| `hyperion_lib` | `<project_name_snake>_lib` |
| `Hyperion` | PascalCase project name |
| `BhagirathsinhRana378` | User's GitHub username |
| `hyperion` | User's project name |
